...
- **미완성 구현**: 이벤트 버스·추론 엔진·메모리 계층은 동작하며 TODO/FIXME/`NotImplemented`가 일부 남아 있습니다(`scripts/setup_packaging.py` 고급 검증, `cognitive/memory/longterm.py`의 비-SQLite 어댑터 확장 등). 과거 실험적 프롬프트 의존 경로는 대부분 정리되었으나 잔존 가능성이 있습니다.
- **핵심 모듈**: CLI/GUI, 도구 안전 정책, Gemini LLM 연동, 정책 병합 경로는 작동합니다. (이벤트 버스 연결, 도구 속도 제한, 기본 협업 정책 템플릿, 배포 스모크 테스트는 2025-10-02 기준 반영 완료)

### ⚠️ 남아 있는 주요 차단 과제
1. **LLM 다중 공급자/고급 폴백 미구현** – 현재는 Gemini 중심이며 응답 병합·모델 스위치 자동화는 로드맵 필요.
2. **대화 맥락 장기 보존 한계** – 긴 세션에서 맥락 손실을 낮추기 위한 외부 메모리·요약 품질 개선 필요.
3. **운영 자동화 보강** – 배포 파이프라인/GUI 회귀 전체 자동화 및 대시보드 연동 고도화 필요.
4. **보안/구성 관리** – 협업 정책 배포/키 관리 프로세스 문서화 및 기본값 검증 강화 필요.
...
- **이벤트 버스 연동 완료**: `PacaSystem._setup_event_handlers()`가 인지·추론·서비스 모듈에 이벤트 버스를 주입하고, 주요 이벤트를 수집·지표화합니다.
- **도구 속도 제한 정책 적용**: `SafetyPolicy`에 호출 간격/쿨다운/소비 API를 추가하고 `PACAToolManager`가 정책 기반 제한을 강제합니다.
- **배포 스모크 테스트 자동화**: `scripts/setup_packaging.py`가 생성된 실행 파일/CLI를 호출해 종료 코드·표준 출력을 검증합니다.
- **협업 정책 기본 템플릿 제공**: `paca_python/config/collab_policy.json` 기본 임계값/채널 구성을 제공해 로더가 항상 유효한 정책을 로드합니다.
- **장기 메모리 어댑터 폴백**: 지원되지 않는 어댑터 지정 시 SQLite(in-memory)로 안전 폴백되어 중단을 방지합니다.

### 🔁 추가 확인 사항
- 다중 LLM 및 응답 병합 로드맵 수립
- 장기 맥락 보존 품질 개선 실험
- 배포 파이프라인/GUI 회귀 자동화
- 협업 정책 배포 및 키 관리 프로세스 정비
