...
- **미완성 구현**: 이벤트 버스·추론 엔진·메모리 계층은 동작하지만, 일부 핵심 경로는 여전히 임시 구현(TODO/FIXME)과 실험용 프롬프트에 의존합니다.
- **핵심 모듈**: CLI/GUI, 도구 안전 정책, Gemini LLM 연동, 정책 병합 등이 가동되지만, 다중 LLM 전환·장기 학습 파이프라인·배포 검증 자동화는 진행 중입니다.
