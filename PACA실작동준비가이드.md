...
- **미완성 구현**: 이벤트 버스·추론 엔진·메모리 계층은 동작하지만 TODO/FIXME/`NotImplemented`가 남아 있으며(`scripts/setup_packaging.py` 실행 검증 TODO, `cognitive/memory/longterm.py`의 비-SQLite 어댑터 등), 일부 경로는 여전히 실험용 프롬프트에 의존합니다.
- **핵심 모듈**: CLI/GUI, 도구 안전 정책, Gemini LLM 연동, 정책 병합 등 주요 경로는 작동하나, 이벤트 버스 연결·도구 속도 제한 등 일부 함수는 과거에는 `pass` 상태여서 보완이 필요했고 다중 LLM 전환·장기 학습 파이프라인·배포 검증 자동화는 여전히 진행 중입니다.

### ⚠️ 남아 있는 주요 차단 과제
1. **LLM 다중 공급자/고급 폴백 미구현** – 현재는 Gemini 단일 경로와 규칙 기반 폴백만 제공되어, 다른 모델 또는 단계적 응답 병합이 불가능합니다.
